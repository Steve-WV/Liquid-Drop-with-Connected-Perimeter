...
struct Dijk {
	struct tri_dist_larger : public std::binary_function< const Tri*, const Tri*, bool> {
		tri_dist_larger() {}
		inline bool operator()(const Tri* t1, const Tri* t2) const;
	};
	double dist, u_av, length;
	Tri* pre;
	bool closed, in_supp;
	boost::heap::fibonacci_heap <Tri*, boost::heap::compare<tri_dist_larger> >::handle_type handle; //i really hate this.
};
...
public:
	...
	inline Tri() { 
		...
		inline void Calc_grad_v( Vec2& )const;
		//inline void Calc_gradPhi( Vec2& ) const;
		
		inline Dijk& dijk() { return dijk_data; }
		inline const Dijk& dijk() const { return dijk_data; } 
...
#include "vertex.hpp"
	...
	inline void
	Tri::Calc_grad_v(Vec2& grad_v) const {
		grad_v = a()->v() * grad_s[0];
		grad_v += b()->v() * grad_s[1];
		grad_v += c()->v() * grad_s[2];
	}
...
/*inline void
	...
	inline bool Dijk::tri_dist_larger::operator()(const Tri* t1, const Tri* t2) const {
		return (*t1).dijk().dist > (*t2).dijk().dist;
	}
	/*
	inline void 
	Tri::Calc_Ftinv() {
